...
def plate(rng: random.Random, valid: bool) -> str:
    ...
    letters = "".join(rng.choice("ABCDEFGHJKLMNPRSTUVYZ") for _ in range(3))
    return f"{province:02d} {letters} {rng.randrange(100, 1000)}"


def render(text: str, font_path: str, rng: random.Random, difficulty: float) -> Image.Image:
    ...
    canvas = Image.new("RGB", (900, 200), (238, 238, 234))
    draw = ImageDraw.Draw(canvas)
    box = draw.textbbox((0, 0), text, font=font)
    draw.text(((900 - (box[2] - box[0])) // 2, (200 - (box[3] - box[1])) // 2 - box[1]),
...
def build(out: pathlib.Path, items: int, difficulties: list[float], seed: int) -> dict:
    font_path = find_font()
    ...
    manifest = {"seed": seed, "items": items, "difficulties": difficulties,
                "font": font_path, "count": len(stimuli), "stimuli": stimuli}
